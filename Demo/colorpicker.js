...
(function (Raphael) {
    ...
    var doc = document, win = window,
        ColorPicker = function (x, y, size, initcolor, element) {
            ...
            t.disc.drag(function (dx, dy, x, y) {
                ...
                t.hsOnTheMove = true;
                var targ = element;
                var off_x = targ.offsetLeft;
                var off_y = targ.offsetTop;
                while(targ.offsetParent){
                    targ = targ.offsetParent
                    off_x += targ.offsetLeft;
                    off_y += targ.offsetTop;
                }
                off_x += -40;
                off_y += -20;
                t.setHS(x + scrollX - t.x- off_x, y + scrollY - t.y - off_y);
            }, function () {
                t.hsOnTheMove = false;
            });
    ...
    ColorPicker.prototype.docOnMove = function (dx, dy, x, y) {
        ...
        if (this.hsOnTheMove) {
            this.setHS(x + scrollX - this.x - off_x, y + scrollY - this.y - off_y);
        }
        if (this.bOnTheMove) {
            this.setB(x + scrollX - this.x - off_x);
        }
    };
